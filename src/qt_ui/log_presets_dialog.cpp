...
LogPresetsDialog::LogPresetsDialog(std::shared_ptr<GUISettings> gui_settings, QWidget* parent)
...
void LogPresetsDialog::LoadFromSettings() {
    ...
    QList<QString> list;
    if (var.isValid()) {
        list = m_gui_settings->Var2StringList(var);
    }
...
void LogPresetsDialog::SaveToSettings() {
    ...
    m_gui_settings->SetValue(kPresetsGroup, kPresetsKey, m_gui_settings->StringList2Var(list));
}
